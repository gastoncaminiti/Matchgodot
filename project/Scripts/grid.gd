	...
	for i in width:
		for j in height:
			if !restricted_movement(Vector2(i,j)): 
				#chouse random number and store it
				...
				# Intance that piece from the array
				add_child(piece)
...
func match_at(i, j, color):
	if i > 1:
		var p1 = all_pieces[i - 1][j]
		var p2 = all_pieces[i-2][j]
		if p1 != null && p2 != null:
			if p1.color == color && p2.color == color:
				return true
	if j > 1:
		var p3 = all_pieces[i][j-1]
		var p4 = all_pieces[i][j-2]
		if p3 != null && p4 != null:
			if p4.color == color && p4.color == color:
				return true
...
func swap_back():
	...
	#Move the previously swapped pieces back to the place.
	state = move
	move_checked = false
	pass
